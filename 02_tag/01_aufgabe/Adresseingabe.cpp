...
Adresseingabe::Adresseingabe(QWidget * parent) : QWidget(parent)
{
	...
	m_name = new QLineEdit("Mustermann", this); // ab morgen last parameter this
	m_vorname = new QLineEdit("Max", this);
	m_anrede = new QComboBox(this);
	m_strasse = new QLineEdit("Street", this);
	m_hausnummer = new QLineEdit("13", this);
	m_PLZ = new QLineEdit("12345", this);
	m_stadt = new QLineEdit("De", this);
...
const bool Adresseingabe::validateInput() const 
{
	...
	if(!fehler.isEmpty()){
		...
		QMessageBox msgBox;
		msgBox.critical(0, "Error", fehler);
